...
hmapUI <- function(id) {
	ns <- NS(id)
	tagList(
	   plotOutput(ns("up_plot")),			#remove comma if commenting out below
	   verbatimTextOutput(ns("test_url"))  		#<---------------------UNDER CONSTRUCTION DEBUG
	)
}
...
hmapServer <- function(id, common) {
 moduleServer(id, function(input, output, session) { 
  ...
  my_URL        <- reactive({ common$myURL })          	#<---------------------UNDER CONSTRUCTION	
  output$test_url <- (renderPrint ({common$myURL}) )	#<---------------------UNDER CONSTRUCTION	
