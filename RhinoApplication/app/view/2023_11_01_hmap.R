...
#' @export
hmapServer <- function(id, data) {
 moduleServer(id, function(input, output, session) { 
  
  #up_file_path <- "/huge1/cstorer/ThemeMap/thememap_sserv/RhinoApplication/app/toy_data"
    
  req(upload$uploadServer("file_path"))  #<---------------------------
  up_file_path <- upload$uploadServer("file_path")  #<----------------
